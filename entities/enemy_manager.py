...
class EnemyManager:
    def __init__(self, game):
        ...
        self.particles = [] 
        self.spawn_timer = 0
        self.spawn_delay = 3.5 # Slightly more breathing room
        
        # Boss Systems - Pushed much further back (8km to 15km)
    ...
    def update(self, dt, player_pos, proj_manager, difficulty_mult):
        ...
        if self.boss_active:
            ...
            if self.sky_alpha > 0:
                self.sky_alpha = max(0, self.sky_alpha - 40 * dt)

        # Enemies move faster based on difficulty
        ...
        # Spawning logic with "Soft Scaling"
        if self.game.state == "PLAYING" and not self.boss_active and self.sky_alpha < 50:
            self.spawn_timer += dt
            
            # Using Square Root makes spawn rate increases less aggressive
    ...
    def trigger_death_effect(self, x, y):
        ...
        for _ in range(count): self.particles.append(DeathParticle(x, y))
